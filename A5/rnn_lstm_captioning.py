...
class ImageEncoder(nn.Module):
    ...
    def __init__(self, pretrained: bool = True, verbose: bool = True):
        ...
        """
        super().__init__()
        self.cnn = torchvision.models.regnet_x_400mf(weights=RegNet_X_400MF_Weights.DEFAULT)
...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    z = x @ Wx + prev_h @ Wh + b.reshape(1, -1)
    next_h = torch.tanh(z)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(seq_len):
        h, cache = rnn_step_forward(x[:,i,:], h, Wx, Wh, b)
        h_list.append(h)
...
def rnn_backward(dh, cache):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    (x, prev_h, next_h, Wx, Wh) = cache[-1]
    N,T,H = dh.shape
    D = x.shape[1]
    dx = torch.zeros((N,T,D)).to(x)
    dh0 = torch.zeros_like(prev_h)
    dWx = torch.zeros_like(Wx)
    dWh = torch.zeros_like(Wh)
    db = torch.zeros(H).to(dx)
    
    d_hi = 0
    for i in reversed(range(len(cache))):
        d_hi += dh[:,i,:]
        d_xi, d_hi, d_Wx, d_Wh, d_b = rnn_step_backward(d_hi, cache[i])
        dx[:,i,:] = d_xi
        dWh += d_Wh
        dWx += d_Wx
        db += d_b
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    # Replace "pass" statement with your code
    N, T, V = x.shape
    loss = F.cross_entropy(x.reshape(N*T,V), y.reshape(N*T),
                           ignore_index=ignore_index, reduction='none').reshape(N,T)
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        if cell_type == 'rnn':
            ...
            self.feat_proj = nn.Conv2d(self.encoder.out_channels, hidden_dim,kernel_size=1)
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type in ['rnn', 'lstm']:
            image_feat = image_feat.reshape(N, C, -1).mean(dim=-1)
        h0 = self.feat_proj(image_feat)
        emb = self.embedding(captions_in)
        # print(h0.shape, emb.shape)
        h = self.rnn(emb, h0)
        scores = self.out_proj(h)
        # print(h.shape, scores.shape)
    ...
    def sample(self, images, max_length=15):
        ...
        for i in range(max_length - 1):
            emb = self.embedding(captions[:, i])
            if self.cell_type == 'rnn':
                next_h = self.rnn.step_forward(emb, prev_h)
            elif self.cell_type == 'lstm':
                next_h, next_c = self.rnn.step_forward(emb, prev_h, prev_c)
                prev_c = next_c
            else:
                attn, attn_weights = dot_product_attention(prev_h, A)
                next_h, next_c = self.rnn.step_forward(emb, prev_h, prev_c, attn)
                prev_c = next_c
                attn_weights_all[:, i, :, :] = attn_weights
            prev_h = next_h
            scores = self.out_proj(next_h)
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        A = x.mm(self.Wx) + prev_h.mm(self.Wh) + self.b.reshape(1,-1)
        H = prev_h.shape[1]
        i = F.sigmoid(A[:, 0:H])
        f = F.sigmoid(A[:, H:2*H])
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        ######################################################################
        (N, H), T = h0.shape, x.shape[1]
        hn = torch.zeros(N,T,H).to(h0)
        # Replace "pass" statement with your code
        prev_h, prev_c = h0, c0
        for i in range(T):
...
def dot_product_attention(prev_h, A):
    ...
    # Replace "pass" statement with your code
    A = A.reshape(N,H,-1)
    M = torch.einsum('NHI,NH->NI', A, prev_h) / math.sqrt(H) # N, 16
    attn_weights = F.softmax(M, dim=1) # N, 16
    attn = torch.einsum('NHI,NI->NH', A, attn_weights) # N,H
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        next_h, next_c = None, None
        # Replace "pass" statement with your code
        A = x.mm(self.Wx) + prev_h.mm(self.Wh) + attn.mm(self.Wattn) + self.b.reshape(1,-1)
        H = prev_h.shape[1]
        i = F.sigmoid(A[:, 0:H])
        f = F.sigmoid(A[:, H:2*H])
        o = F.sigmoid(A[:, 2*H:3*H])
