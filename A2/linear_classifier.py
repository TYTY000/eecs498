...
def svm_loss_vectorized(
    ...
    loss = 0.0
    scores = X.mm(W)
    margin = scores - scores[torch.arange(N), y].reshape(-1,1) + 1  # note delta = 1
    margin[margin<0] = 0
    ...
    # Replace "pass" statement with your code
    margin[margin>0] = 1
    margin[torch.arange(N), y] -= torch.sum(margin, dim=1)
    dW = X.t().mm(margin)
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    randidx = torch.randint(0, num_train, (batch_size, ))
    X_batch, y_batch = X[randidx], y[randidx]
    #########################################################################
    #                       END OF YOUR CODE                                #
    #########################################################################
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [i*10**j for i in range(1,5,2) for j in range(-5,-2)]
    regularization_strengths = [10**i for i in range(-7,-3)]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    train_acc = (y_train == y_pred).float().mean().item()
    y_pred = cls.predict(X_val)
    val_acc = (y_val == y_pred).float().mean().item()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i])
        scores -= scores.max()
        expsum = scores.exp().sum()
        loss += (torch.log(expsum) - scores[y[i]])
        for j in range(num_classes):
            if j == y[i]:
                dW[:, j] += (torch.exp(scores[j]) / expsum * X[i] - X[i])
            else:
...
def softmax_loss_vectorized(
    ...
    pred_minus = torch.zeros_like(scores)
    pred_minus[torch.arange(N), y] = -1
    dW += X.t().mm(pred_minus)
    dW += X.t().mm(exp / expsum.reshape(-1,1))
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [i*10**j for i in range(1,6,2) for j in range(-4,-1)]
    regularization_strengths = [10**i for i in range(-7,-4)]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
