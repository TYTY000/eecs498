...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    layer1 = X.mm(W1) + b1
    hidden = torch.max(layer1, torch.zeros_like(layer1))
...
def nn_forward_backward(
    ...
    scores -= scores_max.reshape(-1,1)
    exp = scores.exp()
    expsum = exp.sum(dim=1)
    loss += (torch.log(expsum) - scores[torch.arange(N), y]).sum()
    loss /= N
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    grad_softmax = exp / expsum.reshape(-1,1)
    grad_softmax[torch.arange(N), y] -= 1
    grad_softmax /= N

    grads['W2'] = h1.t().mm(grad_softmax) + 2 * reg * W2
    grads['b2'] = grad_softmax.sum(dim=0)
    grad_relu = grad_softmax.mm(W2.t())
    grad_relu[h1<=0] = 0
    grads['W1'] = X.t().mm(grad_relu) + 2 * reg * W1
    grads['b1'] = grad_relu.sum(dim=0) 
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [random.uniform(1e-5, 1e-2) for _ in range(5)]
    hidden_sizes = [random.choice([64, 128, 256, 384]) for _ in range(5)]
    regularization_strengths = [random.uniform(1e-8, 1e-6) for _ in range(5)]
    learning_rate_decays = [random.uniform(0.9, 1) for _ in range(5)]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    lrs, hds, rss, lrds = get_param_set_fn()
    ins = data_dict['X_train'].shape[1]
    outs = max(data_dict['y_train']).item() + 1
    print(outs)
    for lr in lrs:
        for hd in hds:
            for rs in rss:
                for lrd in lrds:
                    model = TwoLayerNet(ins, hd, outs)
                    hyper = model.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'], lr, lrd, rs, )
                    y_pred = model.predict(data_dict['X_val'])
                    val_acc = (data_dict['y_val'] == y_pred).float().mean().item()
                    if best_val_acc < val_acc:
                        best_net, best_stat, best_val_acc = model, hyper, val_acc
                        print(lr, hd, rs, lrd)
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
