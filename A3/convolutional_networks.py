...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        out = torch.zeros((N,F,H_,W_), dtype=x.dtype, device=x.device)
        for n in range(N):
            for f in range(F):
                for h_ in range(H_):
                    for w_ in range(W_):
                        h0, w0 = h_ * stride, w_ * stride
                        window = x_pad[n, :, h0:h0+HH, w0:w0+WW]
                        out[n,f,h_,w_] = torch.sum(window * w[f]) + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
            ...
            for f in range(F):
                for h_ in range(H_):
                    for w_ in range(W_):
                        ...
                        window = x_pad[n, :, h0:h0+HH, w0:w0+WW]
                        # out[n,f,h_,w_] = torch.sum(window * w[f]) + b[f]
                        dx_pad[n, :, h0:h0+HH, w0:w0+WW] += w[f] * dout[n, f, h_, w_]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        H_, W_ = 1 + (H - pool_height) // stride, 1 + (W - pool_width) // stride
        out = torch.zeros((N,C,H_,W_), dtype=x.dtype, device=x.device)
        for n in range(N):
            for c in range(C):
                for h_ in range(H_):
                    for w_ in range(W_):
                        ...
                        out[n,c,h_,w_] = torch.max(window)
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
            ...
            for c in range(C):
                for h_ in range(H_):
                    for w_ in range(W_):
                        ...
                        dx[n,c,h0:h0+pool_height, w0:w0+pool_width][mask] = dout[n,c,h_,w_]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        # Replace "pass" statement with your code
        C,H,W = input_dims
        self.params['W1'] = torch.normal(0, weight_scale, 
        ...
        self.params['W2'] = torch.normal(0, weight_scale, 
                                         (num_filters * H//2 * W//2, hidden_dim), dtype=dtype, device=device)
        self.params['b2'] = torch.zeros(hidden_dim, dtype=dtype, device=device)
        self.params['W3'] = torch.normal(0, weight_scale, 
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        layer1, cache1 = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        layer2, cache2 = Linear_ReLU.forward(layer1, W2, b2)
        scores, cache3 = Linear.forward(layer2, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        ds, dW3, db3 = Linear.backward(grad_softmax, cache3)
        dW3 += self.reg * 2 * W3
        d2, dW2, db2 = Linear_ReLU.backward(ds, cache2)
        dW2 += self.reg * 2 * W2
        dX, dW1, db1 = Conv_ReLU_Pool.backward(d2, cache1)
        dW1 += self.reg * 2 * W1
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        if weight_scale == 'kaiming':
            self.params['W1'] = kaiming_initializer(
                    C, num_filters[0], filter_size, relu=True, dtype=dtype, device=device)
        else:
            self.params['W1'] = torch.normal(0, weight_scale, 
                                         (num_filters[0], C, filter_size, filter_size), dtype=dtype, device=device)
        self.params['b1'] = torch.zeros(num_filters[0], dtype=dtype, device=device)
        for i in range(2, self.num_layers):
            if weight_scale == 'kaiming':
                self.params[f'W{i}'] = kaiming_initializer(num_filters[i-2], num_filters[i-1], filter_size, 
                                                    relu=True, dtype=dtype, device=device)
            else:
                self.params[f'W{i}'] = torch.normal(0, weight_scale, 
                                                (num_filters[i-1], num_filters[i-2], filter_size, filter_size), 
                                                dtype=dtype, device=device)
            self.params[f'b{i}'] = torch.zeros(num_filters[i-1], dtype=dtype, device=device)
        scaling = 2**len(max_pools)
        last_dim = (H // scaling) * (W // scaling) * num_filters[-1]
        if weight_scale == 'kaiming':
            self.params[f'W{self.num_layers}'] = kaiming_initializer(last_dim, num_classes, None, 
                                                    relu=False, dtype=dtype, device=device)
        else:
            self.params[f'W{self.num_layers}'] = torch.normal(0, weight_scale,
                                            (last_dim, num_classes), dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        dx, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(grad_softmax, caches[-1])
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            grads[f'W{i}'] += self.reg * 2 * self.params[f'W{i}']
        #############################################################
        #                       END OF YOUR CODE                    #
        #############################################################
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.normal(0, (gain / Din)**0.5, (Din, Dout), dtype=dtype, device=device)
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.normal(0, (gain / Din / K / K)**0.5, (Dout, Din, K, K), dtype=dtype, device=device)
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
