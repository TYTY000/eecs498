...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        layer1, cache1 = Linear_ReLU.forward(X, W1, b1)
        scores, cache2 = Linear.forward(layer1, W2, b2)
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        ###################################################################
        # Replace "pass" statement with your code
        loss, gloss = softmax_loss(scores, y)
        loss += self.reg * (torch.sum(W1*W1) + torch.sum(W2*W2))
        dh, dw2, db2 = Linear.backward(gloss, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        - reg: Scalar giving L2 regularization strength.
        - weight_scale: Scalar giving the standard devhidden_dimiation for random
          initialization of the weights.
        ...
        for i in range(1, self.num_layers + 1):
            self.params[f'W{i}'] = torch.normal(0.0, weight_scale, (dims[i-1], dims[i]), dtype=dtype, device=device)
            self.params[f'b{i}'] = torch.zeros(dims[i], dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        cache.append(cache0)
        if self.use_dropout:
            h, cache1 = Dropout.forward(h, self.dropout_param)
            cache_dropout.append(cache1)
        for i in range(2, self.num_layers):
            h, cache0 = Linear_ReLU.forward(h, self.params[f'W{i}'], self.params[f'b{i}'])
            cache.append(cache0)
            if self.use_dropout:
                h, cache1 = Dropout.forward(h, self.dropout_param)
                cache_dropout.append(cache1)
        scores, cache0 = Linear.forward(h, self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'])
        cache.append(cache0)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        dh, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(grad_loss, cache[-1])
...
def get_five_layer_network_params():
    ...
    # model achieves 100% training accuracy within 20 epochs.      #
    ################################################################
    learning_rate = 1.8e-1  # Experiment with this!
...
def rmsprop(w, dw, config=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    v = (1 - config['decay_rate']) * (dw**2) + config['decay_rate'] * config['cache']
    next_w = w - (config['learning_rate'] * dw) / (v.sqrt() + config['epsilon'])
    config['cache'] = v
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    next_w = w - (config['learning_rate'] * mt) / (vt.sqrt() + config['epsilon'])
    config['m'] = m
    config['v'] = v
    #########################################################################
    #                              END OF YOUR CODE                         #
