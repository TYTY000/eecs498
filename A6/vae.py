...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        # Replace "pass" statement with your code
        out = self.encoder(x)
        mu, logvar = self.mu_layer(out), self.logvar_layer(out)
...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        inp = torch.cat((x.reshape(x.shape[0], -1), c), dim=1)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    reconstructed_loss = F.binary_cross_entropy(x_hat, x, reduction='sum')
    kl_loss = -0.5 * torch.sum(1 + 2 * logvar - mu ** 2 - torch.exp(logvar))
    loss = (reconstructed_loss + kl_loss) / x.shape[0]
    ###############################################################################
    #                            END OF YOUR CODE                                 #
    ###############################################################################
    return loss
