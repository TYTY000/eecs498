...
def compute_saliency_maps(X, y, model):
    ...
    score = model(X)
    loss = F.cross_entropy(score, y, reduction='sum')
    loss.backward()
    grad = X.grad.data
    saliency, _ = torch.abs(grad).max(dim=1)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return saliency
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for iter in range(max_iter):
        ...
        pred_score = score[0, target_y]
        model.zero_grad()
        pred_score.backward()
        with torch.no_grad():
            grad = X_adv.grad.data
            X_adv += learning_rate * grad / torch.linalg.norm(grad)
        if verbose:
            print(f"iteration {iter}, \tpred_score:{pred_score.item():.3f}, \ttrue_class:{target_y}, \tpred_class:{pred.item()} ")
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    score = model(img)
    pred_score = score[0, target_y]
    loss = pred_score - l2_reg * torch.sum(img**2)
    loss.backward()
