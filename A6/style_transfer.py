...
def content_loss(content_weight, content_current, content_original):
    ...
    assert(content_current.shape[0] == 1)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, l in enumerate(style_layers):
        feat = feats[l]
        g = gram_matrix(feat)
        loss += (style_weights[i] * (g - style_targets[i]) ** 2).sum()
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def guided_gram_matrix(features, masks, normalize=True):
    ...
    features = features.reshape(N,R,C,-1)
    masks = masks.reshape(N,R,-1)
    gram = torch.zeros(N,R,C,C).to(features)
    for r in range(R):
        F_r = features[:,r,:,:] * masks[:,r,:]
        G_r = torch.bmm(F_r, F_r.transpose(-2,-1))
        if normalize:
            G_r /= (H * W * C)
        gram[:,r,:,:] = G_r
    return gram
...
def guided_style_loss(feats, style_layers, style_targets, style_weights, content_masks):
    ...
    for i, l in enumerate(style_layers):
        feat = feats[l]
        mask = content_masks[l]
        g = guided_gram_matrix(feat, mask)
        loss += (style_weights[i] * (g - style_targets[i]) ** 2).sum()
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
