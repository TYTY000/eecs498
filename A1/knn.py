...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    xtr = x_train.clone().reshape(num_train, -1)
    xte = x_test.clone().reshape(num_test, -1)
    for i in range(num_train):
        for j in range(num_test):
            dists[i][j] = ((xtr[i] - xte[j]) ** 2).sum() ** 0.5
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    xte = x_test.clone().reshape(num_test, -1)
    for i in range(num_train):
        dists[i, :] = ((xtr[i] - xte) ** 2).sum(dim=1) ** 0.5
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    xtr = x_train.clone().reshape(num_train, -1)
    xte = x_test.clone().reshape(num_test, -1)
    dists = (xtr ** 2).sum(dim=1).reshape(-1,1) + (xte ** 2).sum(dim=1).reshape(1,-1) - 2 * torch.mm(xtr, xte.t())
    dists = dists.sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    top, topidx = torch.topk(dists, k=k, dim=0, largest=False)
    pred = y_train[topidx]
    print(pred.size())
    for i in range(num_test):
        labels, counts = torch.unique(pred[:,i], dim=0, return_counts=True)
        y_pred[i] = labels[torch.argmax(counts)]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            xtr = torch.cat(x_train_folds[:i] + x_train_folds[i+1:])
            ytr = torch.cat(y_train_folds[:i] + y_train_folds[i+1:])
            xte, yte = x_train_folds[i], y_train_folds[i]
            classifier = KnnClassifier(xtr, ytr)
            k_to_accuracies[k].append(classifier.check_accuracy(xte, yte, k))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    # Replace "pass" statement with your code
    best_acc = 0
    for k, accu in k_to_accuracies.items():
        avg_accu = sum(accu) / len(accu)
        if best_acc < avg_accu:
            best_acc, best_k = avg_accu, k
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
