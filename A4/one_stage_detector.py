...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        in_ = in_channels
        for out_ in stem_channels:
            conv_cls = nn.Conv2d(in_, out_, kernel_size=3, padding=1, stride=1, bias=True)
            nn.init.normal_(conv_cls.weight, 0, 0.01)
            if conv_cls.bias is None:
                conv_cls.bias = torch.zeros(out_, requires_grad=False)
            else:
                nn.init.constant_(conv_cls.bias, 0)
            stem_cls.append(conv_cls)
            stem_cls.append(nn.ReLU())

            conv_box = nn.Conv2d(in_, out_, kernel_size=3, padding=1, stride=1, bias=True)
            nn.init.normal_(conv_box.weight, 0, 0.01)
            if conv_box.bias is None:
                conv_box.bias = torch.zeros(out_, requires_grad=False)
            else:
        ...
        self.pred_cls = nn.Conv2d(in_, num_classes, kernel_size=3, stride=1, padding=1, bias=True)   # Class prediction conv
        self.pred_box = nn.Conv2d(in_, 4, kernel_size=3, stride=1, padding=1, bias=True)             # Box regression conv
        self.pred_ctr = nn.Conv2d(in_, 1, kernel_size=3, stride=1, padding=1, bias=True)             # Centerness conv

        nn.init.normal_(self.pred_cls.weight, 0, 0.01)
        if self.pred_cls.bias is None:
            self.pred_cls.bias = torch.zeros(num_classes, requires_grad=False)
        else:
            nn.init.constant_(self.pred_cls.bias, 0)
        nn.init.normal_(self.pred_cls.weight, 0, 0.01)

        if self.pred_box.bias is None:
            self.pred_box.bias = torch.zeros(4, requires_grad=False)
        else:
            nn.init.constant_(self.pred_box.bias, 0)
        nn.init.normal_(self.pred_box.weight, 0, 0.01)

        if self.pred_ctr.bias is None:
            self.pred_ctr.bias = torch.zeros(1, requires_grad=False)
        else:
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for k, v in feats_per_fpn_level.items():
            class_logits[k] = self.pred_cls(self.stem_cls(v))
            class_logits[k] = class_logits[k].reshape(class_logits[k].shape[0], class_logits[k].shape[1], -1).permute(0,2,1)
            boxreg_deltas[k] = self.pred_box(self.stem_box(v))
            boxreg_deltas[k] = boxreg_deltas[k].reshape(boxreg_deltas[k].shape[0], 4, -1).permute(0,2,1)
            centerness_logits[k] = self.pred_ctr(self.stem_box(v))
            centerness_logits[k] = centerness_logits[k].reshape(centerness_logits[k].shape[0], 1, -1).permute(0,2,1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_match_locations_to_gt(
    ...
    for level_name, centers in locations_per_fpn_level.items():
        ...
        x, y = centers.unsqueeze(dim=2).unbind(dim=1) # HW,2 => HW,1
        x0, y0, x1, y1 = gt_boxes[:, :4].unsqueeze(dim=0).unbind(dim=2) # M,4 => 4 * 1,M
        pairwise_dist = torch.stack([x - x0, y - y0, x1 - x, y1 - y], dim=2) # HW,M,4

        # Pairwise distance between every feature center and GT box edges:
        # shape: (num_gt_boxes, num_centers_this_level, 4)
        pairwise_dist = pairwise_dist.permute(1, 0, 2) # M, HW, 4

        # The original FCOS anchor matching rule: anchor point must be inside GT.
        match_matrix = pairwise_dist.min(dim=2).values > 0 # M,HW,1
        ...
        match_quality, matched_idxs = match_matrix.max(dim=0) # 1, HW, 1  | most matched object
        matched_idxs[match_quality < 1e-5] = -1 # match threshold = 1e-5
...
def fcos_get_deltas_from_locations(
    ...
    # Replace "pass" statement with your code
    locations = locations.tile(1,2)
    deltas = torch.abs(locations - gt_boxes[:,:4]) / stride
    bg = -torch.ones(4).to(gt_boxes)
    deltas[torch.all(gt_boxes == bg, dim=1)] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    output_boxes = torch.zeros_like(deltas, dtype=locations.dtype, device=locations.device)
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = torch.sqrt(
            torch.min(lr, dim=1).values * torch.min(tb, dim=1).values /
            torch.max(lr, dim=1).values / torch.max(tb, dim=1).values,
            ).to(dtype=deltas.dtype, device=deltas.device)

    judge = -torch.ones(4).to(device=deltas.device)
    centerness[(deltas == judge)[:,1]] = -1
    # Replace "pass" statement with your code
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        locations_per_fpn_level = get_fpn_location_coords(shape_per_fpn_level, strides_per_fpn_level, device=images.device, dtype=images.dtype)
        ...
        for batch in range(gt_boxes.shape[0]):
            ...
            for level, matched_boxes in matched_boxes_per_level.items():
                batch_match_boxes[level] = matched_boxes
                batch_match_deltas[level] = fcos_get_deltas_from_locations(
                        locations_per_fpn_level[level], matched_boxes, strides_per_fpn_level[level])
            matched_gt_boxes.append(batch_match_boxes)
            matched_gt_deltas.append(batch_match_deltas)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        # Replace "pass" statement with your code
        matched_gt_classes = F.one_hot(
                (matched_gt_boxes[:, :, 4] + 1).to(dtype=int), num_classes=21)[:,:,1:].to(
                        dtype=matched_gt_boxes.dtype, device=matched_gt_boxes.device)
        loss_cls = sigmoid_focal_loss(inputs=pred_cls_logits, targets=matched_gt_classes)
        ...
        loss_ctr = F.binary_cross_entropy_with_logits(
            pred_ctr_logits.view(-1), gt_centerness, reduction="none"
        )
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 2:
            # Replace "pass" statement with your code
            confidant = level_pred_scores > test_score_thresh
            level_pred_scores = level_pred_scores[confidant]
            level_pred_classes = classes[confidant]

            # Step 3:
            # Replace "pass" statement with your code
            level_pred_boxes = fcos_apply_deltas_to_locations(
                    level_deltas, level_locations, self.backbone.fpn_strides[level_name])
            level_pred_boxes = level_pred_boxes[confidant]

            # Step 4: Use `images` to get (height, width) for clipping.
            # Replace "pass" statement with your code
            level_pred_boxes[:, 0].clamp_(min=0)
            level_pred_boxes[:, 1].clamp_(min=0)
            level_pred_boxes[:, 2].clamp_(max=images.shape[2])
            level_pred_boxes[:, 3].clamp_(max=images.shape[3])
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
